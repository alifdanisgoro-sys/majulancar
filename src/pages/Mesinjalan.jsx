...
export default function Mesinjalan() {
  ...
  // 🔹 Ambil kategori unik
  const categories = ["Semua", ...new Set(products2.map((p) => p.category))]

  // 🔹 Filter produk berdasarkan pencarian & kategori
  let filteredProducts2 = products2.filter((product2) => {
    const matchCategory = selectedCategory === "Semua" || product2.category === selectedCategory
    const matchSearch = product2.name.toLowerCase().includes(searchTerm.toLowerCase())
    return matchCategory && matchSearch
  })

  // 🔹 Urutkan produk (sorting)
  if (sortOption === "az") {
    filteredProducts2.sort((a, b) => a.name.localeCompare(b.name))
  } else if (sortOption === "za") {
    filteredProducts2.sort((a, b) => b.name.localeCompare(a.name))
  } else if (sortOption === "kategori") {
    filteredProducts2.sort((a, b) => a.category.localeCompare(b.category))
  }
  ...
  const currentProducts2 = filteredProducts2.slice(indexOfFirstItem, indexOfLastItem)
    ...
    <section className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-16 px-6">
      ...
      {currentProducts2.length > 0 ? (
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
          {currentProducts2.map((product2) => (
            <div
              key={product2.id}
              onClick={() => navigate(`/mesinjalan/${product2.id}`)}
            ...
            >
              <img src={product2.image} alt={product2.name} className="w-full h-56 object-cover" />
              <div className="p-4">
                <span className="text-sm font-semibold text-gray-500">{product2.category}</span>
                <h2 className="text-xl font-bold text-blue-700">{product2.name}</h2>
              </div>
            </div>
          ))}
        </div>
